...
- Concept Showcase page (2026-09-05): /simulations/preview — hero with cyan→violet gradient "in motion.", reserved 16:9 video slot ("Your demo video goes here", data-testid demo-video-slot — swap in user's MP4 later), three animated in-code work mocks (ScopeSortMini chip-sort loop, LoopMini waste-score ring, ReportMini section-tick compliance) each with bullets + notify links, waitlist CTA. Touchpoints rewired to it: Home vision "See a Concept Preview", Simulations hero "See a Concept Preview", all 3 concept cards get "See the work in motion" links
