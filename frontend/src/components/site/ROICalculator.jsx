...
const FRAMEWORKS_COVERED = {
  "Single Framework (e.g., just GRI or just CSRD)": 1,
  "Multiple Frameworks": 3,
  "Full Multi-Framework + Third-Party Assurance": 6,
};

const READINESS_IMPROVEMENT_PCT = {
  "Static Materials (PDFs / Slide Decks)": 0.35,
  "Blended (Some Interactive Elements)": 0.22,
  "Fully Interactive (Simulation-Based)": 0.12,
};

const HOURS_SAVED_PER_EMPLOYEE = {
...
function MetricTile({ label, value, testid, big = false }) {
  return (
    <div className={`rounded-xl bg-white p-5 ${big ? "sm:col-span-2" : ""}`} data-testid={testid}>
      ...
      <p className={`mt-2 font-display font-black tracking-tight text-ink ${big ? "text-3xl md:text-4xl" : "text-2xl"}`}>
        {value}
      </p>
    </div>
  );
...
export default function ROICalculator() {
  ...
  const m = useMemo(() => {
    ...
    return {
      modeledWorkforce, businessUnits, annualComplianceFindings, frameworksCovered,
      readinessImprovement, findingsAvoided, findingCostSavings,
      low: mid * 0.85, high: mid * 1.15, roiMultiple, paybackMonths,
    };
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/contact`, {
        ...
        body: JSON.stringify({
          ...
          last_name: "-",
          email: lead.email,
          topic: "ROI Report Request",
          message: `ROI report request for ${lead.company}. Inputs — workforce: ${band}; industry: ${industry}; method: ${method}; scope: ${scope}. Model: savings ${fmt$(m.low)}–${fmt$(m.high)}, ROI ${m.roiMultiple.toFixed(1)}x, payback ${m.paybackMonths}mo.`,
        }),
      });
  ...
  return (
    <section id="roi-calculator" className="bg-forest-light px-6 py-24 lg:px-10 lg:py-28" data-testid="roi-section">
      <div className="mx-auto max-w-4xl">
        <Reveal className="text-center">
          <Tag className="justify-center">ROI Calculator</Tag>
          <h2 className="mx-auto mt-4 max-w-2xl font-display text-3xl font-extrabold tracking-tight text-ink md:text-4xl text-balance">
            The business case for <span className="text-forest">simulation-based</span> sustainability training.
          </h2>
          <p className="mx-auto mt-4 max-w-xl text-base leading-relaxed text-body">
            Answer a few questions about your organization and see the potential impact of switching to
            Green Mind Learning.
          </p>
        </Reveal>
                ...
                {step === 2 && (
                  <div data-testid="roi-step-2">
                    ...
                    <div className="mt-7 grid gap-5 sm:grid-cols-2">
                      ...
                      <Field label="Reporting framework scope" value={scope} onChange={setScope} options={SCOPES} testid="roi-field-scope" />
                    </div>
                ...
                {step === 3 && (
                  <div data-testid="roi-step-3">
                    ...
                    <p className="mt-5 text-xs leading-relaxed text-slate-400" data-testid="roi-disclaimer">
                      Estimates are illustrative, based on industry benchmarks and the inputs provided. Actual
                      results vary by organization.
                    </p>
